...
def load_data(root, num_query, num_train, batch_size, num_workers):
    ...
    Returns
        train_dataloader, query_dataloader, retrieval_dataloader(torch.evaluate.data.DataLoader): Data loader.
    """
    CIFAR10.init(root, num_query, num_train)
    train_dataset = CIFAR10('train', transform=train_transform(), target_transform=Onehot(10))
    query_dataset = CIFAR10('query', transform=query_transform(), target_transform=Onehot(10))
    retrieval_dataset = CIFAR10('database', transform=query_transform(), target_transform=Onehot(10))
